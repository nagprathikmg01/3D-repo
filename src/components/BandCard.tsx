...
import { Environment, Lightformer, useGLTF, useTexture, Sparkles } from "@react-three/drei";
...
function CursorLight() {
  ...
  useFrame((state) => {
    if (lightRef.current) {
      // Scale coordinates from canvas space to 3D space
      const x = state.pointer.x * 6;
      const y = state.pointer.y * 6;
      lightRef.current.position.set(x, y, 3);
    }
  });
  return <pointLight ref={lightRef} intensity={7} distance={12} color="#06b6d4" decay={2} />;
}
